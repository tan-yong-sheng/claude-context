...
import { OpenAIEmbedding, OpenAIEmbeddingConfig, VoyageAIEmbedding, VoyageAIEmbeddingConfig, OllamaEmbedding, OllamaEmbeddingConfig, GeminiEmbedding, GeminiEmbeddingConfig, SqliteVecConfig, SplitterType, SplitterConfig, AstCodeSplitter, LangChainCodeSplitter } from '@tan-yong-sheng/code-context-core';

// Simplified sqlite-vec configuration interface for frontend
export interface SqliteVecWebConfig {
    dbPath?: string;
}
...
export interface PluginConfig {
    ...
    vectorDbConfig?: SqliteVecWebConfig;
...
export class ConfigManager {
    ...
    /**
     * Get vector database (sqlite-vec) frontend configuration
     */
    getVectorDbConfig(): SqliteVecWebConfig | undefined {
        const config = vscode.workspace.getConfiguration(ConfigManager.CONFIG_KEY);
        const dbPath = config.get<string>('vectorDb.dbPath');
    ...
    /**
     * Save vector database (sqlite-vec) frontend configuration
     */
    async saveVectorDbConfig(vectorDbConfig: SqliteVecWebConfig): Promise<void> {
        if (!vectorDbConfig) {
            throw new Error('Vector database config is undefined');
        }

        const workspaceConfig = vscode.workspace.getConfiguration(ConfigManager.CONFIG_KEY);

        await workspaceConfig.update('vectorDb.dbPath', vectorDbConfig.dbPath ?? undefined, vscode.ConfigurationTarget.Global);
    }

    /**
     * Convert frontend configuration to complete SqliteVecConfig
     */
    getVectorDbFullConfig(): SqliteVecConfig | undefined {
        const webConfig = this.getVectorDbConfig();
        if (!webConfig) return undefined;

        // Convert simplified frontend config to complete config
        return {
            dbPath: webConfig.dbPath
        };
    }
